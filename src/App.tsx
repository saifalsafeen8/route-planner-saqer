...
import { addStopAction, clearStops, removeStopAction, reorderStops, setStopsAction, updateStopPosition } from './store/stopsSlice'
...
export default function App() {
  const stops = useAppSelector(s => s.stops.items);
  ...
  useEffect(() => {
    ...
    const stopProgs = stops.map(s => {
      ...
      return closestIndex / coords.length
    });

    let currentStopIndex = 0
    for (let i = 0; i < stopProgs.length; i++) {
      if (sim.progress >= stopProgs[i]) {
        currentStopIndex = i
      } else {
        break
      }
    ...
    setCurrentStopId(stops[currentStopIndex].id)
  ...
  const addStop = useCallback(async (lng: number, lat: number, name?: string, address?: string) => {
    ...
    dispatch(addStopAction(stop))
    reset()
  }, [stops.length, reset, dispatch])

  const _removeStop = useCallback((id: string) => {
    dispatch(removeStopAction(id))
    reset();
  }, [reset]);

  const _reorder = useCallback((from: number, to: number) => {
    dispatch(reorderStops({ from, to }))
    reset()
  }, [reset]);

  const _onMarkerDrag = useCallback(async (id: string, lng: number, lat: number) => {
    let snapped = { lng, lat }
    if (hasToken()) {
      try {
    ...
    const addr = hasToken() ? await reverseGeocode(snapped.lng, snapped.lat) : `${snapped.lat.toFixed(4)}, ${snapped.lng.toFixed(4)}`
    dispatch(updateStopPosition({ id, lng: snapped.lng, lat: snapped.lat, address: addr }))
  }, []);
  ...
  const handleOptimize = useCallback(async () => {
    ...
    dispatch(setStopsAction(result.optimizedStops))
    reset()

  }, [stops, reset]);

  const fmtDist = (meter: number) => meter < 1000 ? `${Math.round(meter)}m` : `${(meter / 1000).toFixed(1)} km`
  const fmtTime = (second: number) => {
    const h = Math.floor(second / 3600), m = Math.round((second % 3600) / 60)
    ...
    <div className="h-screen w-screen flex flex-col bg-dark-900 text-gray-200 overflow-hidden" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <div className="h-11 bg-[#16213f] backdrop-blur border-b border-dark-600 flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          {stops.length !== 0 &&
            < button onClick={() => { dispatch(clearStops()); reset() }} className="text-xs text-gray-500 hover:text-red-400">Clear all</button>
          }
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-[340px] bg-[#16213f] border-r border-dark-600 flex flex-col shrink-0 overflow-hidden">
          ...
          <div className="px-4 py-1.5 flex items-center justify-between bg-[#0e172a] mb-2">
          ...
          <div className="flex-1 overflow-y-auto px-1.5 scrollbar-thin">
            <StopsList stops={stops} onReorder={_reorder} onRemove={_removeStop}
            ...
            <div className="px-4 bg-[#1a1a2e] py-3 border border-[#8b5cf6] rounded-lg shrink-0 m-1.5">
              ...
              <div className="flex items-center gap-2 mb-2.5">
                <button onClick={sim.status === 'playing' ? pause : play}
                  className="w-8 h-8 rounded-md bg-[#8b5cf6] hover: hover:bg-[#5917f3] flex items-center justify-center text-white text-sm font-bold transition-colors">
                  {sim.status === 'playing' ? '⏸' : '▶'}
                </button>
        ...
        <div className="flex-1 relative">
          <MapView stops={stops} route={route} simProgress={sim.progress} simStatus={sim.status}
            onMapClick={(lng, lat) => addStop(lng, lat)} onMarkerDrag={_onMarkerDrag} />
        </div>
      </div>
